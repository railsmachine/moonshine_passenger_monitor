...
* Edit moonshine.yml to customize plugin settings as desired:
    :passenger_monitor
      ...
      # Rack
      # :pattern: 'Rack: /srv/#{configuration[:application]}/current'
      
      # pre-Rack
      # :pattern: 'Rails: /srv/#{configuration[:application]}/current'
      
      # proctitle, port can be set or defaults to 80
      # :pattern: '[#{configuration[:application]}/(#{options[:port] || 80)}]'
